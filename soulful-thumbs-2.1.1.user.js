...
(function(){
  ...
  function main(){
    ...
    // State
    let state={ url:location.href, dirs:[], images:[], files:[], filtered:{dirs:[],images:[],files:[]} };

    function updateUpDisabled(){ const upBtn=$('#st-up'); upBtn.disabled=isRootUrl(state.url); }

    function breadcrumb(url){ const bc=$('#st-bc'); bc.innerHTML=''; const u=new URL(url); const segs=u.pathname.split('/').filter(Boolean); let acc=u.origin+'/'; if(segs.length===0){ bc.appendChild(span('/')); return; } for(let i=0;i<segs.length;i++){ bc.appendChild(span('›','st-crumb-sep')); acc+=segs[i]+'/'; const a=document.createElement('a'); a.href=acc; a.textContent='/'+segs[i]; a.addEventListener('click',e=>{ e.preventDefault(); navigateTo(acc); }); bc.appendChild(a);} }
    function span(t,cls){ const s=document.createElement('span'); s.textContent=t; if(cls) s.className=cls; return s; }

    function info(){ const total=state.dirs.length+state.images.length+state.files.length; $('#st-info').textContent=`${total} items • ${state.dirs.length} dirs • ${state.images.length} imgs • ${state.files.length} files`; }

    function filterAndSort(){
      const q=($('#st-q').value||'').trim().toLowerCase(); const scope=$('#st-scope').value;
      const pass=n=>!q || n.toLowerCase().includes(q);
      const sort=$('#st-sort').value; const sortBy={ 'name-asc':(a,b)=>nat(a.name,b.name), 'name-desc':(a,b)=>nat(b.name,a.name), 'type-asc':(a,b)=>{ const ta=ext(a), tb=ext(b); return nat(ta,tb)||nat(a.name,b.name);} }[sort]||((a,b)=>nat(a.name,b.name));
      const ext=o=> (o.url.split('.').pop()||'').toLowerCase();

      const wantDirs = scope==='all' || scope==='dirs';
      const wantImgs = scope==='all' || scope==='imgs';
      const wantFiles= scope==='all' || scope==='files';

      state.filtered.dirs   = wantDirs ? state.dirs.filter(d=>pass(d.name)).sort((a,b)=>nat(a.name,b.name)) : [];
      state.filtered.images = wantImgs ? state.images.filter(i=>pass(i.name)).sort(sortBy) : [];
      state.filtered.files  = wantFiles? state.files.filter(i=>pass(i.name)).sort(sortBy) : [];
    }

    function render(){ filterAndSort(); info(); updateUpDisabled(); const view=$('#st-view').value; if(view==='grid'){ renderGrid(); list.style.display='none'; grid.style.display='grid'; } else { renderList(); grid.style.display='none'; list.style.display='block'; } }

    function folderSVG(){ return `<svg class="st-folder" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 6.5A2.5 2.5 0 0 1 5.5 4h3.6a2.5 2.5 0 0 1 1.77.73l1.42 1.54c.47.5 1.12.78 1.8.78H18.5A2.5 2.5 0 0 1 21 9.5v6A2.5 2.5 0 0 1 18.5 18h-13A2.5 2.5 0 0 1 3 15.5v-9Z" stroke="var(--st-ac)" stroke-opacity=".9" stroke-width="1.2"/></svg>`; }

    // --- GRID (event delegation fixes the “can’t click while searching” bug) ---
    function renderGrid(){
      grid.innerHTML='';
      for(const d of state.filtered.dirs){ const card=document.createElement('div'); card.className='st-item dir'; card.dataset.url=d.url; card.innerHTML=`${folderSVG()}<div class="st-dirname" title="${d.name}">${d.name}</div>`; grid.appendChild(card); }
      state.filtered.images.forEach((img)=>{ const a=document.createElement('div'); a.className='st-item img'; a.innerHTML=`<img loading="lazy" decoding="async" src="${img.url}" alt="${img.name}" data-url="${img.url}"><div class="st-caption" title="${img.name}">${img.name}</div>`; grid.appendChild(a); const im=$('img',a); im.addEventListener('load',()=>meta.noteImageSize(img.url, im.naturalWidth, im.naturalHeight)); meta.observe(im); });
      state.filtered.files.forEach((f)=>{ const el=document.createElement('div'); el.className='st-item img'; el.innerHTML=`<div style="display:flex;align-items:center;justify-content:center;min-height:calc(var(--st-size) - 2px);font-size:calc(var(--st-label)*1.1);color:var(--st-fg);">${f.name}</div>`; el.dataset.url=f.url; el.dataset.file='1'; grid.appendChild(el); });
    }

    // Click delegation
    grid.addEventListener('click',(e)=>{
      const dirCard=e.target.closest('.st-item.dir'); if(dirCard){ navigateTo(dirCard.dataset.url); return; }
      const imgCard=e.target.closest('.st-item.img'); if(!imgCard) return; const img=e.target.closest('img');
      if(img){ // open in lightbox (index is by filtered.images order)
        const url=img.getAttribute('data-url'); const idx=state.filtered.images.findIndex(x=>x.url===url); if(idx>=0) lb.openAt(idx); return;
      }
      // non-image file tile: open in new tab
      if(imgCard.dataset.file==='1'){ const u=imgCard.dataset.url; if(u) window.open(u,'_blank','noopener'); }
    });

    // --- LIST --- (same as before, simplified here)
    function renderList(){ list.innerHTML=''; const h=document.createElement('div'); h.className='st-row st-h'; h.innerHTML=`<div class="st-name">Name</div><div>Type</div><div>Size</div><div>Date</div>`; list.appendChild(h);
      for(const d of state.filtered.dirs){ const r=document.createElement('div'); r.className='st-row'; r.innerHTML=`<div class="st-name">${folderSVG()}<span class="st-dirname" title="${d.name}">${d.name}</span></div><div>dir</div><div>—</div><div>—</div>`; r.addEventListener('click',()=>navigateTo(d.url)); list.appendChild(r); }
      const both=[...state.filtered.images.map(x=>({...x,_t:'image'})), ...state.filtered.files.map(x=>({...x,_t:'file'}))];
      for(const it of both){ const r=document.createElement('div'); r.className='st-row'; const ext=(it.url.split('.').pop()||'').toLowerCase(); r.innerHTML=`<div class="st-name"><span class="st-dirname" title="${it.name}">${it.name}</span></div><div>${it._t==='image'?'image':ext}</div><div>—</div><div>—</div>`; r.addEventListener('click',()=>window.open(it.url,'_blank','noopener')); list.appendChild(r); }
      lb.setGetter((idx)=> state.filtered.images[idx]);
    }

    // Navigation
    async function navigateTo(url, opts={push:true}){
      try{ const doc=(url===location.href && document) || await fetchDoc(url); if(!looksLikeAutoIndex(doc)){ location.href=url; return; } const {dirs,images,files}=parseIndex(doc,url); state.url=url; state.dirs=dirs; state.images=images; state.files=files; breadcrumb(url); render(); if(opts.push) history.pushState({url},'',url); }
      catch(err){ console.error('[Soulful Thumbs] nav fail',err); location.href=url; }
    }
    window.addEventListener('popstate',(e)=>{ const to=(e.state&&e.state.url)||location.href; navigateTo(to,{push:false}); });

    // Wire toolbar
    $('#st-home').addEventListener('click',()=>navigateTo(homeUrl(state.url)));
    $('#st-up').addEventListener('click',()=>{ if(!isRootUrl(state.url)) navigateTo(parentUrl(state.url)); });
    $('#st-raw').addEventListener('click',()=>window.open(state.url,'_blank','noopener'));

    const setCssVar = (name, val)=>document.documentElement.style.setProperty(name, val);
    $('#st-size').addEventListener('input',(e)=>{ setCssVar('--st-size', e.target.value+'px'); localStorage.setItem(LSK.size,e.target.value); });
    $('#st-gap').addEventListener('input',(e)=>{ setCssVar('--st-gap', e.target.value+'px'); localStorage.setItem(LSK.gap,e.target.value); });
    $('#st-label').addEventListener('input',(e)=>{ setCssVar('--st-label', e.target.value+'px'); localStorage.setItem(LSK.label,e.target.value); });

    $('#st-sort').addEventListener('change',(e)=>{ localStorage.setItem(LSK.sort,e.target.value); render(); });
    $('#st-scope').addEventListener('change',(e)=>{ localStorage.setItem(LSK.scope,e.target.value); render(); });
    $('#st-view').addEventListener('change',(e)=>{ localStorage.setItem(LSK.view,e.target.value); render(); });
    $('#st-theme').addEventListener('change',(e)=>{ localStorage.setItem(LSK.theme,e.target.value); if(e.target.value==='contrast') document.documentElement.classList.add('st-contrast'); else document.documentElement.classList.remove('st-contrast'); });
    $('#st-q').addEventListener('input', debounce(render, 120));

    // One‑tap hidden items toggle
    $('#st-showhidden').addEventListener('click',()=>{
      const id='st-reveal-css'; const exists=$('#'+id);
      if(exists){ exists.remove(); original.forEach(el=>el.classList.add('st-hide-original')); $('#st-showhidden').textContent='Show hidden'; return; }
      original.forEach(el=>el.classList.remove('st-hide-original'));
      const rev=document.createElement('style'); rev.id=id; rev.textContent='*[hidden], a[hidden]{display:inline!important} a, pre a, table a{display:inline!important;visibility:visible!important;opacity:1!important;max-height:none!important}'; document.documentElement.appendChild(rev);
      $('#st-showhidden').textContent='Hide revealed';
    });

    // Expand All button for sitemap
    $('#st-expandall').addEventListener('click',()=>{
      try{
        const nodes=document.querySelectorAll('.st-tree details'); nodes.forEach(d=>{ d.open=true; });
        console.log('[Soulful Thumbs] Expand All: unleashed your inner explorer!');
      }catch(ex){ console.warn('Expand All clicked, but no tree found:', ex); }
    });

    // Initial
    const init=parseIndex(document, location.href); state.url=location.href; state.dirs=init.dirs; state.images=init.images; state.files=init.files; breadcrumb(state.url); render();
  }

  try{ main(); } catch(err){ console.error('[Soulful Thumbs] init failed:', err); }
})();