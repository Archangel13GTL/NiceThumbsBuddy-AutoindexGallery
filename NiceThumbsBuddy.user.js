// ==UserScript==
// @name         NiceThumbsBuddy — Autoindex & Local Directory Gallery
// @namespace    archangel.nicethumbsbuddy
// @version      3.0.0
// @description  Transform bare Apache/Nginx directory listings and local file browser pages into a rich, accessible gallery with smart folders, interactive sitemap, advanced sorting, zoom, and local previews.
...
(function () {
  ...
  const AUDIO_EXT = /\.(mp3|wav|flac|ogg)$/i;
  const DOC_EXT = /\.(pdf|txt|docx?|xlsx?|pptx?)$/i;
  const ARCHIVE_EXT = /\.(zip|rar|7z|tar|gz)$/i;
  const FILE_EXT = /\.(avif|webp|jpe?g|png|gif|bmp|svg|heic|tif?f|mp4|mov|webm|mkv|pdf|zip|rar|7z|tar|gz|mp3|wav|flac|ogg|txt|docx?|xlsx?|pptx?)$/i;
  ...
  // --------------------------- Detection Logic ----------------------------
  function isFunctionalLink(a) {
    return a && a.href && !a.href.startsWith('mailto:') && !a.href.startsWith('javascript:');
  }
  function isDirHref(href) {
    return /\/$/.test(href.split('#')[0].split('?')[0]);
  }
  function isImgHref(href) {
    return IMG_EXT.test(href.split('?')[0]);
  }
  function isVideoHref(href) {
    return VIDEO_EXT.test(href.split('?')[0]);
  }
  function isAudioHref(href) {
    return AUDIO_EXT.test(href.split('?')[0]);
  }
  function isDocHref(href) {
    return DOC_EXT.test(href.split('?')[0]);
  }
  function isArchiveHref(href) {
    return ARCHIVE_EXT.test(href.split('?')[0]);
  }
  function isFileHref(href) {
    return FILE_EXT.test(href.split('?')[0]);
  }
  ...
  function parseIndex(doc, baseUrl) {
    ...
    for (const a of anchors) {
      ...
      if (dirs.length + images.length + videos.length + audios.length + docs.length + archives.length + files.length >= MAX_ITEMS_PAGE) break;
    }
    ...
    return {
      ...
      videos: uniq(videos).sort(byName),
      audios: uniq(audios).sort(byName),
      docs: uniq(docs).sort(byName),
      archives: uniq(archives).sort(byName),
    ...
    async function fetchMeta(url) {
      ...
      const promise = (async () => {
        try {
          ...
          const meta = {
            mtime: res.headers.get('last-modified') || '',
          ...
          if (meta.mtime) {
            ...
            if (!isNaN(+d)) meta.mtime = d.toISOString();
          }
          if (meta.size) meta.size = parseInt(meta.size, 10);
          if (meta.type) meta.type = meta.type.split(';')[0].trim();

          // Cache the result
          cache.set(url, meta);
          sessionStorage.setItem(storageKey, JSON.stringify(Object.fromEntries(cache)));
          sessionStorage.setItem(storageExpiry, (Date.now() + CACHE_EXPIRY).toString());

          return meta;
        } catch (e) {
          console.error(`[NiceThumbsBuddy] Error fetching metadata for ${url}:`, e);
          return {};
        } finally {
          inFlight.delete(url);
        }
  ...
  function init() {
    ...
    if (typeof GM_addStyle === 'function') {
      GM_addStyle(/* css */`
        /* ...modern CSS for grid, list, toolbar, lightbox, sitemap, accessibility... */
      `);
    }
    // ...existing code to parse index, render UI, setup event handlers...
    // setupAccessibility();
    // setupSettingsPanel();
  }
